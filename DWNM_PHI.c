...
double complex* phi_values_omega_domain(double complex omegas[], int size) {
    double complex* phi_values = malloc(size * sizeof(double complex));
    ...
    for (int i = 0; i < size; i++) {
        ...
        for (int n = 0; n < N_RANGE; n++) {
            ...
            if (n == 0) {
                phi_sum += (-I * M_PI / L) * (k / nu) * j0(k * R) * cexp(-I * nu * fabs(Z));
            } else {
                phi_sum += (2 * (-I * M_PI / L) * (k / nu) * j0(k * R) * cexp(-I * nu * fabs(Z)));
            }
        }
